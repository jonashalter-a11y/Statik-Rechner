...
function getVerificationsForNorm(normId) {
  ...
  return rows.map(v => {
    ...
    // graph_json als STRING (nicht geparst!)
    if (v.graph_json) {
...
for (const norm of norms) {
  ...
  if (norm.id === 'sia265') {
    ...
    console.log(`✓ seed-chapters.js`);
    console.log(`✓ seed-verifications.js (${verifs.filter(v => v.graph_json).length}/${verifs.length} mit graph_json)`);
  } else {
  ...
  verifications: ${JSON.stringify(verifs, null, 2)}
    ...
    console.log(`✓ seed-${norm.id}.js (${verifs.filter(v => v.graph_json).length}/${verifs.length} mit graph_json)`);
  }
}
