...
function JsonImportModal({ normId, chapterId, chapters, onClose, onImported }: { normId: string; chapterId: string; chapters: Chapter[]; onClose: () => void; onImported: () => void; }) {
  ...
  const doImport = async () => {
    ...
    setSaving(true); setErr('');
    try {
      await api.createTable({ norm_id: normId, chapter_id: chapterId, title: title.trim(), description, type: 'chart', headers: [], rows: [], chart_json: preview });
      onImported(); onClose();
    } catch (e: any) {
...
// ── Haupt-Komponente ───────────────────────────────────────────────────────────
export default function DataTableAdmin({ mode = 'table' }: { mode?: 'table' | 'chart' }) {
  ...
  const load = async () => {
    const [tbls, chs] = await Promise.all([
      (api as any).getTables(normId) as Promise<TableMeta[]>,
      (api as any).getChapters(normId) as Promise<Chapter[]>,
    ]);
    setAllTables(tbls);
    setChapters(chs);
  };
  ...
  const selectTable = async (t: TableMeta) => {
    const full = await (api as any).getTableFull(t.id);
    setSelected(t.id);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!editing.id) {
        const { id } = await (api as any).createTable(payload) as { id: string };
        setSelected(id);
        setEditing({ ...editing, id });
      } else {
        await (api as any).updateTable(editing.id, payload);
      }
      await load();
      setMsg('✓ Gespeichert');
    } catch { setMsg('⚠ Fehler'); }
  ...
  const del = async (id: string) => {
    ...
    await (api as any).deleteTable(id);
  ...
  const dropOnChapter = async (chapterId: string, tableId: string) => {
    ...
    if (!id) return;
    const full = await (api as any).getTableFull(id);
    await (api as any).updateTable(id, { ...full, chapter_id: chapterId });
    setDragTableId(null);
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1.2fr', gap: 10, marginBottom: 14 }}>
            ...
            <div>
              ...
              <select value={editing.chapter_id || ''} onChange={async e => {
                ...
                setEditing(updated);
                if (updated.id) { await (api as any).updateTable(updated.id, updated); await load(); setMsg('✓ Kapitel gespeichert'); if (newChapterId) setExpanded(prev => new Set(prev).add(newChapterId)); }
              }} style={INP}>
